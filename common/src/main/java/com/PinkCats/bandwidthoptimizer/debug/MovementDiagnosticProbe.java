...
public final class MovementDiagnosticProbe {
    ...
    public static void observeConnectionSend(Channel channel, Packet<?> packet) {
        ...
        DiagnosticLog.warn(
                ...
                positionPacket.getY(),
                positionPacket.getZ(),
                positionPacket.getId(),
                positionPacket.getRelativeArguments(),
                pendingTasks(channel),
                channel.isWritable()
        );
    }

    public static void observeDecodedPackets(
            ChannelHandlerContext context,
            PacketFlow flow,
            List<Object> out,
            int startIndex
    ...
    private static final class MoveWindow {
        ...
        synchronized MoveBurst recordMoves(int moves, long nowNanos) {
            ...
            MoveBurst burst = movePackets >= MOVE_BURST_THRESHOLD
                    ? new MoveBurst(movePackets, TimeUnit.NANOSECONDS.toMillis(Math.max(elapsed, 0L)))
                    : null;
        ...
        synchronized boolean markCorrection(long nowNanos) {
            if (nowNanos - lastCorrectionLogNanos < CORRECTION_LOG_MIN_NANOS) {
                return false;
            }
