...
public enum ClientReconnectPlatform implements ClientReconnectCoordinator.Platform {
    ...
    public boolean isConnectInProgress() {
        return ClientWindowCompat.screen(Minecraft.getInstance()) instanceof ConnectScreen;
    }

    @Override
    public boolean isDisconnectedScreenReady() {
        return ClientWindowCompat.screen(Minecraft.getInstance()) instanceof DisconnectedScreen;
    }

    @Override
    public boolean startConnection(ClientReconnectCoordinator.Target target) {
        Minecraft minecraft = Minecraft.getInstance();
        Screen screen = ClientWindowCompat.screen(minecraft);
        Screen parent = screen == null ? new TitleScreen() : screen;
