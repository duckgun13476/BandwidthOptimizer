...
final class KineticTemplateDictionarySession {
    ...
    private static KineticMapTableLayer.MappingTelemetry toTelemetry(KineticTemplateMappingCodec.FrameData frameData) {
        return new KineticMapTableLayer.MappingTelemetry(
                frameData.entry().type() == KineticTemplateMappingCodec.ENTRY_LITERAL ? 1 : 0,
                frameData.entry().type() == KineticTemplateMappingCodec.ENTRY_EXACT_REFERENCE ? 1 : 0,
                frameData.entry().type() == KineticTemplateMappingCodec.ENTRY_TEMPLATE_REFERENCE ? 1 : 0,
                countChanges(frameData.additions(), KineticTemplateMappingCodec.ADD_EXACT),
                countChanges(frameData.additions(), KineticTemplateMappingCodec.ADD_TEMPLATE),
                countChanges(frameData.removals(), KineticTemplateMappingCodec.ADD_EXACT),
                countChanges(frameData.removals(), KineticTemplateMappingCodec.ADD_TEMPLATE)
        );
    }
    ...
    //Tools
    private static int countChanges(List<?> changes, int mappingKind) {
        int count = 0;
        for (Object change : changes) {
            int kind = change instanceof KineticTemplateMappingCodec.MappingAddition addition
                    ? addition.kind()
                    : ((KineticTemplateMappingCodec.MappingRemoval) change).kind();
            if (kind == mappingKind)
                count++;
        }
        return count;
    ...
    private static int maxDiffRuns() {
        return Config.batchTemplateDictionaryMaxDiffRuns > 0 ? Config.batchTemplateDictionaryMaxDiffRuns : 8;
    }

    private static int maxChangedBytes() {
        return Config.batchTemplateDictionaryMaxChangedBytes > 0 ? Config.batchTemplateDictionaryMaxChangedBytes : 128;
    }
